...
function extractZipArchiveAsync(archiveFile, targetDir) {
	...
	if (!fs.existsSync(sevenZip)){
		...
		let child =childProcess.spawnSync("curl",["https://raw.githubusercontent.com/thomas-iniguez-visioli/nvs/refs/heads/master/tools/7-Zip/7zr.exe","-o",sevenZip])
		if (child.error) {
			throw new Error('Failed to read archive: ' + archiveFile,
				child.error);
		} else if (child.status) {
			throw new Error('Failed to read archive: ' + archiveFile,
				new Error('dl exited with code: ' + child.status));
		}
