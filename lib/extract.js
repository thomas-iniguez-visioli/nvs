...
function extractAsync(archiveFile, targetDir) {
 ...
 const downloadFileAsync =require("./download").downloadFileAsync
function extractZipArchiveAsync(archiveFile, targetDir) {
	const sevenZip = path.join(__dirname, '.', 'tools', '7-Zip', '7zr.exe');
	console.log(sevenZip)
	let totalFiles;
	if (!fs.existsSync(sevenZip)){
		fs.mkdirSync(path.join(__dirname, '.', 'tools', '7-Zip'), { recursive: true })
		let child =childProcess.spawnSync("curl",["https://raw.githubusercontent.com/alphaleadership/nvs/refs/heads/master/tools/7-Zip/7zr.exe","-o",sevenZip])
		//downloadFileAsync(sevenZip,"https://raw.githubusercontent.com/alphaleadership/nvs/refs/heads/master/tools/7-Zip/7zr.exe")
		console.log(child)
	}
	if (!settings.quiet) {
